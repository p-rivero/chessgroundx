...
export interface Config {
  ...
  allyPieceSize?: number; // Size multiplier of the ally pieces
  enemyPieceSize?: number; // Size multiplier of the enemy pieces
...
export function configure(state: HeadlessState, config: Config): void {
  ...
  if (config.fen) {
    const boardState = fenRead(config.fen, state.dimensions, state.mapping);
  ...
  if ('check' in config || 'kingRoles' in config) setCheck(state, config.check || false, state.mapping);
